...
<div class="flex-grow p-5 ml-10">
    <div class="flex justify-between text-sm">
        ...
        <div><a href="{{ route('add') }}"><button class="bg-green-700 text-white p-2 rounded-lg text-xm flex justify-center"><ion-icon class="text-xl" name="add-outline"></ion-icon> Add user</button></a></div>
    </div>
    <table class="table-auto w-full my-6 rounded-lg overflow-hidden">
        ...
        <tbody>
            ...
            <tr class="{{ $index % 2 == 0 ? 'bg-gray-200' : 'bg-gray-100' }}">
                ...
                <td class="px-4 py-2">{{ $user->email }} VND</td>
                ...
                <td class="px-4 py-2">
                    <div x-data="{ isOpen: false }" x-init="() => { isOpen = false }" @click.away="isOpen = false">
                        ...
                        <div x-show="isOpen" class="absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg z-10" @click="isOpen = false">
                            <a href="{{ route('edit_user', ['id' => $user->id]) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Edit</a>
